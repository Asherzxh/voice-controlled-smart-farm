...
const int LIGHT_ZERO_OFFSET      = 900;   // 完全遮光时的ADC读数 (需实测标定)
const int LIGHT_LOW_THRESHOLD   = 200;   // 光照过暗 — 需开补光灯 (校准后的值, 几乎全黑)
const int LIGHT_HIGH_THRESHOLD  = 2600;  // 光照过强 — 需遮光 (3500 - OFFSET)
...
// 自动播报
const int U_MSG_PERSON_PASSING = 20;  // 有人经过 (需在 smartpi.cn 固件中配置对应语音)

// 距离过近自动播报冷却时间 (ms) — 避免持续重复播报
const unsigned long DIST_ALERT_COOLDOWN_MS = 5000;
